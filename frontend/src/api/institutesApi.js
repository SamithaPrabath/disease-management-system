...
export const getAllInstitutesData = async () => {
  try {
    if (IS_BACKEND == "false") {
      ...
      const response = await axios.get(`${BASE_URL}/api/institutes/all`);
      if (response.status === 200) {
        const filteredInstitutes = response.data.data.map(({ id, name, registration_number, email, phone_number, address, province, city }) => ({
          id,
          name,
      ...
      return { status: 400, message: "Failed to fetch institutes list", data: [] };
    }
  } catch (error) {
